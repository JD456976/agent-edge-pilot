...
import { X, CheckCircle2, Clock, Flame, Phone, ClipboardList, ExternalLink } from 'lucide-react';
...
export function EndOfDayReviewDrawer({ open, onClose, overdueTasks, untouchedHotLeads, onLogTouch, onCreateTask, onNavigateToTasks }: Props) {
  ...
  const sortedLeads = useMemo(() => {
    return [...untouchedHotLeads].slice(0, 8);
  }, [untouchedHotLeads]);

  const handleMarkDone = async (taskId: string) => {
    await completeTask(taskId);
    toast({ description: 'Task completed', duration: 2000 });
  };
  ...
  const handleCreateFollowUp = async (task: Task) => {
    ...
    toast({ description: 'Follow-up task created', duration: 2000 });
  };

  const handleLeadFollowUp = async (lead: Lead) => {
    onCreateTask(`Follow up ${lead.name}`, lead.id);
  };
                  ...
                  {sortedOverdue.map(task => (
                    <div key={task.id} className="flex items-start gap-3 p-2.5 rounded-md border border-border bg-background">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <Button size="sm" variant="ghost" className="h-7 text-xs px-2" onClick={() => handleMarkDone(task.id)}>
                          <CheckCircle2 className="h-3 w-3 mr-1" /> Done
                        </Button>
                        <Button size="sm" variant="ghost" className="h-7 text-xs px-2" onClick={() => handleCreateFollowUp(task)}>
                          <ClipboardList className="h-3 w-3 mr-1" /> Follow up
                        </Button>
                      </div>
                    </div>
                  ))}
                  ...
                  {sortedLeads.map(lead => (
                    <div key={lead.id} className="flex items-start gap-3 p-2.5 rounded-md border border-border bg-background">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <Button size="sm" variant="ghost" className="h-7 text-xs px-2" onClick={() => handleLeadFollowUp(lead)}>
