...
import { Phone, MessageSquare, Mail, Home, StickyNote, CalendarPlus, Mic, CheckCircle2 } from 'lucide-react';
...
export function LogTouchModal({ open, onClose, entityType, entityId, entityTitle, onTouchLogged }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ description: 'Something went wrong. Touch not saved.', variant: 'destructive' });
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {saved ? (
          <div className="py-6 flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-medium">Touch logged — {touchType}</p>
            <p className="text-xs text-muted-foreground">
              {note ? `"${note.slice(0, 80)}${note.length > 80 ? '…' : ''}"` : 'No note added'}
            </p>
          ...
          <>
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button size="sm" variant="outline" className="flex-1" onClick={handleClose} disabled={submitting}>
                Cancel
              </Button>
            </div>
          </>
