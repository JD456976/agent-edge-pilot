  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      ...
      <button
        ...
        className={cn(
          'flex items-center gap-2 sm:gap-3 rounded-lg border bg-card p-2 sm:p-3 text-left transition-colors hover:bg-accent/50',
          hasMoneyRisk ? 'border-destructive/30' : 'border-border'
        )}
      >
        <div className={cn(
          'hidden sm:flex h-8 w-8 items-center justify-center rounded-md shrink-0',
        ...
        <div className="min-w-0">
          <p className="text-[9px] sm:text-[10px] uppercase tracking-wider text-muted-foreground leading-none mb-1">Income at Risk</p>
          {hasMoneyRisk ? (
            <p className="text-xs sm:text-sm font-bold text-destructive truncate">{formatCurrency(totalMoneyAtRisk)} at risk</p>
          ) : (
            <p className="text-xs sm:text-sm font-medium text-muted-foreground">All clear</p>
          )}
        </div>
      </button>
      ...
      >
        ...
        <div className="min-w-0">
          <p className="text-[9px] sm:text-[10px] uppercase tracking-wider text-muted-foreground leading-none mb-1">Hottest Lead</p>
          {hasHotLead ? (
            <p className="text-xs sm:text-sm font-bold truncate">
              {topLead.name} <span className="hidden sm:inline text-xs font-normal text-muted-foreground">· {topOpportunity.opportunityScore} heat</span>
            </p>
          ) : (
            <p className="text-xs sm:text-sm font-medium text-muted-foreground">All clear</p>
          )}
      ...
      >
        ...
        <div className="min-w-0">
          <p className="text-[9px] sm:text-[10px] uppercase tracking-wider text-muted-foreground leading-none mb-1">Today's Tasks</p>
